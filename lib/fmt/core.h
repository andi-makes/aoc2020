...
namespace detail {
	...
	// A helper function to suppress "conditional expression is constant"
	// warnings.
	template<typename T>
...
#define FMT_USE_INT128 0
	...
	FMT_CONSTEXPR typename std::make_unsigned<Int>::type
...
namespace detail {
	...
	// Specifies whether S is a string type convertible to
	// fmt::basic_string_view. It should be a constexpr function but MSVC 2017
	// fails to compile it in enable_if and MSVC 2015 fails to compile it as an
	// alias template.
	template<typename S>
	struct is_string :
...
namespace detail {
	...
	/**
	  ...
	  internal class and shouldn't be used directly, only via
	  `~fmt::basic_memory_buffer`. \endrst
	 */
	template<typename T>
	class buffer {
	private:
	...
	public:
		...
		// Tries resizing the buffer to contain *count* elements. If T is a POD
		// type the new elements may not be initialized.
		void try_resize(size_t count) {
			try_reserve(count);
			size_ = count <= capacity_ ? count : capacity_;
		}

		// Tries increasing the buffer capacity to *new_capacity*. It can
		// increase the capacity by a smaller amount than requested but
		// guarantees there is space for at least one additional element either
		// by increasing the capacity or by flushing the buffer if it is full.
	...
	struct arg_data {
		// args_[0].named_args points to named_args_ to avoid bloating
		// format_args. +1 to workaround a bug in gcc 7.5 that causes
		// duplicated-branches warning.
...
#define FMT_TYPE_CONSTANT(Type, constant) \
	...
	struct arg_mapper {
		...
		// We use SFINAE instead of a const T* parameter to avoid conflicting
		// with the C array overload.
		template<typename T>
		FMT_CONSTEXPR auto map(T)
			-> enable_if_t<std::is_pointer<T>::value, int> {
			// Formatting of arbitrary pointers is disallowed. If you want to
			// output a pointer cast it to "void *" or "const void *". In
			// particular, this forbids formatting of "[const] volatile char *"
			// which is printed as bool by iostreams.
	...
	using mapped_type_constant =
		type_constant<decltype(
						  arg_mapper<Context>().map(std::declval<const T&>())),
...
FMT_CONSTEXPR_DECL FMT_INLINE auto
	...
	case detail::type::uint128_type: break;
#endif
...
namespace detail {
	...
	struct is_contiguous_back_insert_iterator<
		std::back_insert_iterator<Container>> : is_contiguous<Container> {};
	template<typename Char>
	struct is_contiguous_back_insert_iterator<buffer_appender<Char>> :
		std::true_type {};

	// A type-erased reference to an std::locale to avoid heavy <locale>
	// include.
	class locale_ref {
	private:
	...
	constexpr unsigned long long encode_types() {
		return static_cast<unsigned>(
				   mapped_type_constant<Arg, Context>::value) |
	...
	// a fallback formatter in one translation unit and an implicit conversion
	// in another (not recommended).
...
namespace detail {
	...
	inline void
		vprint_mojibake(std::FILE*, basic_string_view<Char>, const Args&) {}
...
FMT_INLINE std::basic_string<Char>
